"""Shared global node layout utilities for Task A.

This module must be used by both:

- the Task A data loader when candidate names are converted to indices;
- the Task A models when heterogeneous embeddings are flattened.

Using one shared implementation prevents silent source/target index errors.
"""
