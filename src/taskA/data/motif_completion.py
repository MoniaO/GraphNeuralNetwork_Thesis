...
from taskA.features.pair_basis.motifs import (
    DualGate,
    MotifCompletionTask,
    extract_dual_gates,
    load_truth_graph,
...
def oracle_z_map(cfg: Any, samples: pd.DataFrame | None = None) -> dict[tuple[str, str], str]:
    ...
    raise ValueError(f"Unknown taskA.features.pair_basis.z_mode={z_mode!r}")
...
def latent_triple_map(
    ...
    """
    from taskA.features.pair_basis.motif_registry_v3 import GATE_OUTCOMES
    ...
    for t in tasks:
        ...
        if z_mode in {"random_matched", "random"}:
            from taskA.features.pair_basis.variable_spec import VariableType
            from taskA.features.pair_basis.variable_specs_v3 import VARIABLE_SPECS
            from taskA.features.pair_basis.motif_registry_v3 import DUAL_GATES
            import hashlib
