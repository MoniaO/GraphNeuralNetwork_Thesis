...
# Domyslna lista Twoich 10 pierwotnych endpointow (mozesz nadpisac przez cfg).
...
class TargetedPatientDAGNodeClassifier(nn.Module):
    ...
    Roznica wzgledem gnn_node_clf_simple.py: forward() nie zwraca logitow
    dla WSZYSTKICH wezlow typu target_node_type, tylko dla indeksow
    odpowiadajacych `target_endpoint_names` - w stalej kolejnosci.
    """
    ...
    def __init__(
        ...
        # Ustalenie, KTORE wezly typu target_node_type sa faktycznym targetem.
        ...
        # Lokalne indeksy (w ramach wezlow typu target_node_type) do wyciecia
        # tylko interesujacych nas endpointow z pelnej reprezentacji.
    ...
    def _build_edge_kwargs(self, data: HeteroData):
    ...
    def _select_targets(self, full_tensor: torch.Tensor, batch_size: int, n_endpoint_nodes_per_graph: int) -> torch.Tensor:
        """Wybiera z pelnej reprezentacji/tensora (ulozonej per-graf w batchu)
        tylko indeksy odpowiadajace self.target_local_idx, dla kazdego grafu."""
    ...
    def forward(self, data: HeteroData) -> torch.Tensor:
        """Zwraca logity WYLACZNIE dla wezlow z self.target_endpoint_names,
        w stalej kolejnosci, dla kazdego grafu w batchu."""
