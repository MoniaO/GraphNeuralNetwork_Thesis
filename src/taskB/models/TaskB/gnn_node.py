...
class _PatientDAGGNNBase(nn.Module):
    ...
    def __init__(
        ...
        self.head_in_dim = head_in_dim  # zapamietane - podklasy (np. tryb "typed_concat"
                                          # HCR) moga potrzebowac rozszerzyc glowice o dodatkowe wymiary
...
class TargetedPatientDAGNodeClassifier(_PatientDAGGNNBase):
    ...
    def __init__(
        ...
        if self.use_hcr_wide and self.hcr_wide_mode == "linear":
            ...
            # _PatientDAGGNNBase.__init__ specjalnie do tego celu).
    ...
    def forward(self, data: HeteroData) -> torch.Tensor:
        ...
        if self.use_hcr_wide and self.hcr_wide_mode == "typed_concat":
            # WAZNA ROZNICA wzgledem pozostalych trybow: glowica ma szersze
            # wejscie (head_in_dim + total_hcr_dim), wiec NIE MOZE byc
            # zaaplikowana do calego z_dict[target_node_type] naraz (jak
            # nizej dla linear/nonlinear) - selekcja targetow musi nastapic
            # PRZED glowica, nie po niej.
            if batch_size > 1:
            ...
            hcr_fused = torch.cat(type_embeddings, dim=-1)  # [batch*n_targets, 4*embed_dim]
            fused = torch.cat([target_z, hcr_fused], dim=-1)  # [batch*n_targets, head_in_dim+4*embed_dim]
            return self.head(fused).squeeze(-1)

        # --- tryby "linear"/"nonlinear"/brak HCR: DOTYCHCZASOWE zachowanie,
        # bez zmian - glowica aplikowana na CALYM z_dict, selekcja targetow
        # PO glowicy. ---
