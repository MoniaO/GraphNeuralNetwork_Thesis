...
def get_targeted_pair_evidence_named(
    data: HeteroData,
    target_local_idx: torch.Tensor,
    ...
    """Jak get_targeted_pair_evidence, ale czyta atrybuty o DOWOLNEJ nazwie -
    uzywane dla per-typ-wezla tensorow HCR (hcr_typed_{node_type}_features/
    mask, patrz PARENT_NODE_TYPES w gnn_node.py). Ta sama logika indeksowania
    (offsets+target_local_idx), tylko sciezka do atrybutu jest parametrem
    zamiast na sztywno "hcr_pair_features"/"hcr_pair_mask"."""
    features = getattr(data[target_node_type], feature_attr).float()
    mask = getattr(data[target_node_type], mask_attr).float()
    batch_size = int(data[target_node_type].batch.max().item()) + 1 \
        if hasattr(data[target_node_type], "batch") else 1
    n_per_graph = features.size(0) // max(batch_size, 1)
    offsets = torch.arange(batch_size, device=features.device) * n_per_graph
...
class HCRPairEncoder(nn.Module):
    """Maly, WSPOLDZIELONY nieliniowy koder pary (rodzic, endpoint)"""
    ...
    def pooled_embedding(self, features: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """
        features: [..., max_parents, in_channels]
        mask:     [..., max_parents] (1.0 = prawdziwy rodzic, 0.0 = padding)
        zwraca:   [..., embed_dim] - embedding PRZED kolapsem do skalara.
        """
        ...
        # clamp(min=1.0): endpoint/typ bez ZADNEGO rodzica (mask same zera)
        # nie dzieli przez 0 - dostaje wektor zerowy (neutralny), zamiast NaN.
    ...
    def forward(self, features: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        - dotychczasowe zachowanie, bez zmian, uzywane w trybach linear-owym
        rozszerzeniu ("nonlinear").
        """
        pooled = self.pooled_embedding(features, mask)
