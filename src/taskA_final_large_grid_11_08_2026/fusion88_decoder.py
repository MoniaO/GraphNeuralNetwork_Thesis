"""Common edge decoder for Final Large Grid 11.08.2026.

Stage A: g_stat = zeros(24).
Stage C: g_stat = E_AZ || E_AG || E_ZG from statistical pair encoders.

Literal fusion contract from the protocol:

  q_AG (4*d) -> Linear -> 128 -> GELU -> Dropout(0.2)
              -> Linear -> 64 -> GELU -> LayerNorm
  g_stat in R^24
  [g_graph || g_stat] in R^88
  -> 64 -> GELU -> Dropout(0.2) -> 1 logit
"""
