"""Statistical pair encoders + Fusion88StatDecoder (Stage C / FINAL 14.08.2026).

MLP pair encoder (default):
  D → Linear(D,16) → GELU → LayerNorm(16) → Dropout(0.1) → Linear(16,8) → LayerNorm(8)

KAN twin (FINAL):
  KANLinear(D→8) + LayerNorm(8)  — same latent width as MLP (g_stat still R^24)

Three unshared role encoders (AZ, AG, ZG); role masks → g_stat ∈ R^24.
Graph/fusion heads identical to Fusion88Decoder.
"""
