...
Constructing the input for the GNN requires a carefully defined patient-level graph. The input graph for the node classification task is built from: 
...
These components are assembled into a heterogeneous per-patient graph representation whose elements are summarised in \cref{tab:heterodata_components}.
...
Patient observations are encoded as node features. Figure~\ref{fig:patient_dag} shows an exemplary DAG for one patient. 

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth, height=0.45\textheight, keepaspectratio]{patient_dag_101.png}
    \caption{Exemplary per-patient causal DAG for one patient from the synthetic cohort.}
...
A design issue handled during the experiments is the duplication of the self-transform across relations. 
...
(internal self-loop weights disabled for SAGE/Transformer) and replaces it with a single, explicit, 
...
prediction approach includes a statistical evidence branch---a wide
side-channel that supplies cohort-level dependence statistics.
The branch is a \emph{HCR-inspired} way of representing dependencies
(\cref{subsec:hcr_def}).
...
Figure~\ref{fig:taskB-hcr-arch} summarises the dual-path layout: a heterogeneous GNN producing endpoint embeddings, and an HCR-inspired evidence branch that can be concatenated into the classification head or added at the logit level.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{architecture_slide_multilabel_hcr.png}
\caption{Task~B architecture: heterogeneous GNN path with shared endpoint head, HCR-inspired evidence tensors with pair encoders, and optional concatenation or logit-level fusion.}
...
representation captures the strong signal from direct parents and smoothed signal 
from further layers.
The exposition follows the conceptual progression pairwise dependence, type-specific evidence encoding, higher-order triple-path evidence, and fusion with the GNN representation.

All modes of the statistical branch are summarized in
Table~\ref{tab:fusion_modes} summarises all modes; each mode is described in detail below.
...
learned per-endpoint weight (learned per-endpoint scale).
...
Second, fusion is \emph{endpoint-specific}: a learnable endpoint-specific gate matrix $\in \mathbb{R}^{|\mathcal{V}_{\text{target}}| \times B}$
...
1,3 and 6 to show the trend with the increased number of layers. The reporting convention is that unless 
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth, height=0.45\textheight]{hcr_gate_matrix.pdf}
