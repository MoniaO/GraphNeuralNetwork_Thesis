...
Hetionet~\cite{hetionet}, a biomedical knowledge graph of compounds, genes and diseases,
and {H2GB} {IEEE-CIS-G}~\cite{lin2025h2gb,h2gb}, a heterogeneous graph of card-fraud 
transactions.
Hetionet allows the models to be assessed on a graph structure close to the synthetic task
(a directed heterogeneous graph with multiple target nodes), but it cannot be compared
against external performance metrics because the graph was reconstructed.
{H2GB} {IEEE-CIS-G}, by contrast, does support comparison against external performance
metrics and is also a heterogeneous graph for class-imbalanced node classification,
but it is a single-label task on a bipartite star graph.
Taken together, the two settings test whether extending the GNN with a
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Instance.} A compound takes the role of the patient: the
    ...
    the remaining entities. A compound is therefore not a node of the
...
The project comprises nine datasets across five domains (academia, e-commerce, finance, cybersecurity, 
and social science) and benchmarks 28 baseline models under both properties simultaneously.
For the Task~B benchmark, the card-fraud dataset IEEE-CIS-G was chosen.
...
The original release is tabular and contains $590{,}540$ card transactions, of which $20{,}663$ ($3.50\%$) 
are fraudulent, and an accompanying identity table covers $144{,}233$ of these transactions ($24.4\%$).
The target in dataset is whether a transaction is fraudulent.
...
The final IEEE-CIS-G reconstructure includes $144{,}233$ transactions, , of 
which $11{,}318$ ($7.85\%$) are fraudulent, matching the $12{:}1$ class imbalance in 
Table~1 of~\cite{lin2025h2gb}.
The graph stucture 
 comprises $153{,}880$ nodes of $12$ types and $2{,}873{,}472$ edges of $22$ relation types.
...
Unlike the Hetionet proxy, IEEE-CIS-G construction is copied from H2GB project in order
to perform the comparison with project results. 
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Instance.} A transaction takes the role of the patient.
    The transaction is a graph node and
    carries the $4823$-dimensional feature vector.

  \item \textbf{Target.} The predicted endpoint is the binary fraud label
    \texttt{isFraud} on the transaction node ($7.85\%$ positive).

  \item \textbf{Layer mapping.} Only graph node types are assigned to synthetic
    slots (\cref{tab:ieee-cis-mapping}), by the same roles as in Task~B.
    \texttt{patient\_context} is the stable identity of the instance
    (age, sex, comorbidities on the synthetic graph).
    Here that role is the shared payment identity: \texttt{card1}--\texttt{card6},
    \texttt{addr1}--\texttt{addr2} and email domains group many transactions
    and are not the event being classified.
    \texttt{drug\_exposure} is the observed action (which drug was taken).
    The analogue is \texttt{ProductCD}, the product code of the purchase.
    \texttt{mechanism} is a latent process on the path to the endpoint
    (for example reduced kidney blood flow), not an identity token and not
    the action itself.
    IEEE-CIS-G has no such node type: the fraud modus operandi is not
    materialised, and inventing one would break the H2GB schema.
    Vesta fields (\texttt{V*}, \texttt{C*}, \texttt{D*}, amount) and identity
    fields (\texttt{DeviceType}, \texttt{DeviceInfo}, \texttt{id\_*}) are
    channels of the transaction feature vector, so they are not mapped onto
    layers~4--5 either.

  \item \textbf{Direction.} IEEE-CIS-G is a bipartite star stored as a directed
    heterogeneous graph: each of the eleven transaction--metadata associations
    is kept in both directions (\texttt{to} and \texttt{rev\_to}), giving the
    $22$ relation types of H2GB.
    The reverse copies let messages reach the transaction node; they are not a
    causal layer order as in the synthetic DAG.
\end{enumerate}

\begin{table}[H]
  \centering
  \small
  \caption{Mapping of IEEE-CIS-G \emph{node types} onto the synthetic layer schema.
    Transaction-level variables that live in $x$ are omitted: they are features,
    not entities.}
  \label{tab:ieee-cis-mapping}
  \begin{tabular}{clll}
    \toprule
    Layer & Slot & IEEE-CIS-G node type & Observed \\
    \midrule
    ...
    3 & \texttt{mechanism}                   & --- & no \\
    4 & \texttt{adr\_or\_intermediate\_state} & --- & no \\
    5 & \texttt{observation\_or\_selection}  & --- & no \\
    6 & \texttt{clinical\_endpoint}          & \texttt{isFraud} on \texttt{transaction} & target (label) \\
    \bottomrule
  \end{tabular}
\end{table}
...
The present is to repeat the Task~B \emph{deep and wide} design on the same IEEE-CIS-G
input -- a TransformerConv encoder enriched with the
HCR-inspired statistical evidence branch -- and compare it against models results from H2GB.

Instead of calculating evidence based on parent-endpoint relationship, evidence is
computed as pairwise transaction-level statistics. Each slot pairs one
covariate of that transaction with the fraud label.
The covariates are Vesta and identity columns that are not already graph node
types (\texttt{V*}, \texttt{C*}, \texttt{D*}, amount, and low-cardinality
\texttt{M*}/device dummies).
Card, address, email and \texttt{ProductCD} are the part of GNN and are not considered part of
evidence branch.
Grandparent--parent--endpoint \emph{triple} statistics from the synthetic
pipeline are omitted -- there is no typed three-hop causal chain, and the
HCR slots are not parent nodes.

Pairwise evidence uses the $42$-channel layout of Task~B
(\cref{subsec:hcr_application}): matrix-energy summaries, marginals, type
indicators and the instance activation channel, with leave-one-out coefficients
on the training split.
On the training transactions, $408$ candidate columns pass support and variance
...
form the covariate vocabulary.
Each transaction is then packed into $K=16$ pair-encoder slots, keeping the
highest-ranked active covariates.
A shared pair encoder, as in Task~B, embeds every slot,
mean-pools over the mask, and adds a learned scalar to the fraud logit of the
TransformerConv head.
% The $42$-channel evidence layout is unchanged
% (\cref{subsec:hcr_application}), but the MLP is narrower:
% $42 \rightarrow 16 \rightarrow 4$ here, versus $42 \rightarrow 32 \rightarrow 8$
% in \cref{tab:hparams-common}.
% IEEE-CIS-G has a single endpoint and $K=16$ covariate slots rather than typed
% parent tensors, and the widths follow the IEEE HCR implementation rather than a
% retune of the synthetic encoder.
...
The labelled graph is split chronologically $80/10/10$ by \texttt{TransactionDT},
as in Table~1 of~\cite{lin2025h2gb}: training transactions precede validation,
which precede test.
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Depth $L$ & $2$, $3$, $6$ \\
    Statistical branch & nonlinear HCR, $42$ channels, $K=16$ transaction covariates \\
    Pair encoder & $42 \rightarrow 16 \rightarrow 4$ \\
    %Triple evidence & omitted \\
    Sampler & HGTLoader (H2GB \texttt{hgt} budget) \\
    Class weights & $1$, $5$ \\
    Split & chronological $80/10/10$ (by \texttt{TransactionDT}) \\
    Selection metric & validation binary F1 \\
    Seeds & $42$, $202$, $568$, $20256$, $202567$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
% over five seeds ($42$, $202$, $568$, $20256$, $202567$).
...
$0.011 \pm 0.024$ at $L=2$ and $0.019 \pm 0.025$ at $L=6$.
Residual connections restore a stable graph pathway
($0.297$--$0.307$ test F1; $0.760$--$0.764$ test AUROC).
Adding the statistical evidence branch on top of residuals raises the mean test F1 to
$0.439$--$0.471$
%, but the seed spread remains large
(\cref{fig:ieee-cis-ablation}).
%At $L=3$, three seeds sit near $0.50$ while $20256$ and $568$ stay close to the residual-only band.
...
\begin{table}[H]
  ...
  \begin{tabular}{ccl ccc ccc}
    ...
    \midrule
    2 & no  & ---      & $0.010{\scriptstyle\,\pm\,0.023}$ & $0.590{\scriptstyle\,\pm\,0.017}$ & $0.115{\scriptstyle\,\pm\,0.005}$ & $0.011{\scriptstyle\,\pm\,0.024}$ & $0.643{\scriptstyle\,\pm\,0.006}$ & $0.137{\scriptstyle\,\pm\,0.002}$ \\
    \addlinespace[2pt]
    2 & yes & ---      & $0.393{\scriptstyle\,\pm\,0.005}$ & $0.786{\scriptstyle\,\pm\,0.002}$ & $0.309{\scriptstyle\,\pm\,0.010}$ & $0.307{\scriptstyle\,\pm\,0.003}$ & $0.764{\scriptstyle\,\pm\,0.003}$ & $0.241{\scriptstyle\,\pm\,0.005}$ \\
    2 & yes & HCR 40d  & $0.488{\scriptstyle\,\pm\,0.045}$ & $0.825{\scriptstyle\,\pm\,0.017}$ & $0.469{\scriptstyle\,\pm\,0.077}$ & $0.471{\scriptstyle\,\pm\,0.077}$ & $0.824{\scriptstyle\,\pm\,0.027}$ & $0.436{\scriptstyle\,\pm\,0.094}$ \\
    \midrule
    3 & no  & ---      & $0.284{\scriptstyle\,\pm\,0.013}$ & $0.713{\scriptstyle\,\pm\,0.008}$ & $0.230{\scriptstyle\,\pm\,0.010}$ & $0.231{\scriptstyle\,\pm\,0.030}$ & $0.697{\scriptstyle\,\pm\,0.030}$ & $0.191{\scriptstyle\,\pm\,0.027}$ \\
    \addlinespace[2pt]
    3 & yes & ---      & $0.397{\scriptstyle\,\pm\,0.007}$ & $0.789{\scriptstyle\,\pm\,0.004}$ & $0.313{\scriptstyle\,\pm\,0.011}$ & $0.297{\scriptstyle\,\pm\,0.012}$ & $0.762{\scriptstyle\,\pm\,0.007}$ & $0.245{\scriptstyle\,\pm\,0.012}$ \\
    3 & yes & HCR 40d  & $\mathbf{0.466}{\scriptstyle\,\pm\,0.058}$ & $\mathbf{0.818}{\scriptstyle\,\pm\,0.023}$ & $\mathbf{0.428}{\scriptstyle\,\pm\,0.096}$ & $\mathbf{0.439}{\scriptstyle\,\pm\,0.088}$ & $\mathbf{0.810}{\scriptstyle\,\pm\,0.030}$ & $\mathbf{0.387}{\scriptstyle\,\pm\,0.107}$ \\
    \midrule
    6 & no  & ---      & $0.020{\scriptstyle\,\pm\,0.018}$ & $0.612{\scriptstyle\,\pm\,0.022}$ & $0.125{\scriptstyle\,\pm\,0.007}$ & $0.019{\scriptstyle\,\pm\,0.025}$ & $0.634{\scriptstyle\,\pm\,0.010}$ & $0.142{\scriptstyle\,\pm\,0.007}$ \\
    \addlinespace[2pt]
    6 & yes & ---      & $0.388{\scriptstyle\,\pm\,0.004}$ & $0.788{\scriptstyle\,\pm\,0.005}$ & $0.305{\scriptstyle\,\pm\,0.007}$ & $0.297{\scriptstyle\,\pm\,0.010}$ & $0.760{\scriptstyle\,\pm\,0.007}$ & $0.235{\scriptstyle\,\pm\,0.008}$ \\
    6 & yes & HCR 40d  & $0.463{\scriptstyle\,\pm\,0.056}$ & $0.819{\scriptstyle\,\pm\,0.023}$ & $0.425{\scriptstyle\,\pm\,0.102}$ & $0.440{\scriptstyle\,\pm\,0.103}$ & $0.815{\scriptstyle\,\pm\,0.035}$ & $0.397{\scriptstyle\,\pm\,0.124}$ \\
    \bottomrule
  \end{tabular}%
  }
\end{table}

Additionally, the proposed wide \& deep approach achieves better mean results than
the models reported in the H2GB project.
\Cref{fig:ieee-cis-h2gb} places the $L=3$ HCR model next to the IEEE-CIS-G F1 numbers published
 with H2GB~\cite{lin2025h2gb}.
The comparison is on the same IEEE-CIS-G input 
(HGT own reimplementation matching the 
...
and present run over five seeds like H2GB. 
The $L=3$ HCR mean test F1 is $0.439 \pm 0.088$, above HGT ($0.309$), SHGN ($0.317$)
and $\mathrm{H}^{2}$G-former ($0.316$), but the interval still overlaps that band.
HCR combined with residual connections
effectively boosts the performance over just graph structure.
...
On IEEE-CIS-G the same Task~B pattern holds under a different graph contract:
residual connections stabilize the TransformerConv pathway, and the statistical
evidence branch raises mean test F1 above the graph-only residual model
($0.439 \pm 0.088$ at $L=3$ versus $0.297 \pm 0.012$).
% The mean also lies above the published H2GB F1 band (about $0.31$), on the same
% input and five-seed count, but two of five seeds remain near the residual-only
% regime, so the comparison is a mean gain with a wide interval, not a clean ranking
% win.

In summary, Hetionet and IEEE-CIS-G show that extending the GNN with a
statistical evidence branch remains useful outside the synthetic experiments.
The value of the branch is complementary to residual message passing and depends
on the graph.
