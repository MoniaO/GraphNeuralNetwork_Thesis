...
The experiments assumed to used multiple number of layers as 
...
\cref{tab:arch-nores-update}. All reported metrics are defined in
\cref{subsubsec:oversmoothing}. In every case the epoch is selected on the
validation set and the test metrics are taken from that epoch.

The range of depths follows the assumption that the longest path from a drug exposure to an endpoint in
the audited DAG spans up to six hops, and every node along such a path may
...
%refreshed results with 8 endpoints evaluated
    ...
    performance (\cref{tab:arch-res}): 77--86\% of the Bayes ceiling on the test
  ...
  \begin{tabularx}{\textwidth}{l l >{\raggedright\arraybackslash}X}
    ...
    typed+triple
      ...
      & 42 pairwise channels split across four non-shared encoders, one per
        parent type, each with a learned per-endpoint gate, plus an ungated
        third-order block \\
    \bottomrule
  \end{tabularx}
\end{table}
...
of the Bayes ceiling at $L=1$, $3$ and $6$ respectively. The concatenated
...
\end{table}
    ...
    Based on results of experiments, we can observed that both are effective, but they are not complementary.
...
Averaged over endpoints the gates order as
\texttt{adr\_or\_intermediate\_state} ($0.72$), \texttt{patient\_context}
($0.38$), \texttt{mechanism} ($0.18$) and \texttt{drug\_exposure} ($0.00$),
which is the same order as the number of endpoints that possess a parent of
each type ($11$, $8$, $3$ and $2$ out of $11$). The model therefore recovers
the layout of the causal graph from the gradient alone.
...
To sum up, it is worth to emphasize that all of models with HCR reached a satisfactory results 
comparable to models with residual connections applied. Taking into consideration gate matrix, it 
could be observed that HCR brings value to specific number of endpoints which corresponds with higher
prediction accuracy. 
...
never collapses, retaining between $82\%$ and $89\%$ of the ceiling, while
...
\emph{typed+triple} at $L=1$ and $L=4$ ($89.05\%$ and $86.65\%$ of the Bayes ceiling). 
...
Endpoints are ordered by prevalence. \%~ceil.\ is
    $(\mathrm{AUC}-0.5)/(\mathrm{ceiling}-0.5)$ against the per-endpoint
    Bayes ceiling; values above $100\%$ indicate that the estimate on a
    finite sample exceeds the ceiling computed on the generator, and should
    be read as saturation rather than as exceeding the theoretical limit.
    ...
    \begin{table}[htbp]
      ...
      \begin{tabular}{lccc ccc ccc}
        \toprule
        & & \multicolumn{2}{c}{Ceiling} & \multicolumn{3}{c}{Validation} & \multicolumn{3}{c}{Test} \\
        \cmidrule(lr){3-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
        Endpoint & Prev. & AUC & AUPRC & AUC & AUPRC & \%~ceil. & AUC & AUPRC & \%~ceil. \\
        \midrule
        AKI                & 0.131 & 0.811 & 0.454 & 0.794 & 0.406 & 94.3  & \textbf{0.785} & \textbf{0.421} & 91.6 \\
        ...
        Delirium           & 0.037 & 0.668 & 0.100 & 0.649 & 0.093 & 89.0  & 0.687 & 0.098 & \textbf{111.4} \\
