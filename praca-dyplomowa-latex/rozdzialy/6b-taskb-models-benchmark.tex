...
The gain from HCR is conditional on the state of the propagation path: at $L=6$ without
...
Comparing to results from synthethic datasets, we can see that on the proxy dataset combination of residual connections and 
HCR add up at $L=6$. The baseline
...
The reason is the stucture of data. In the synthetic graph the direct
...
bottleneck. In the Hetionet-derived graph the the depth is more crucial as some of layers are latent. Information must
therefore traverse several hops, and the residual path and the HCR bypass
contribute at different points of that route. 

Overall, combining a graph neural network with an HCR branch improves
prediction on this dataset, and the improvement grows with depth. Whether an
additional residual path helps or hinders depends on where the statistical
