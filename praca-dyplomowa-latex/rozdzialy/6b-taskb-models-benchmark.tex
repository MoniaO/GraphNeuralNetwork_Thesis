...
The benchmark dataset for endpoint prediction is Hetionet knowledge graph described in \cref{subsec:hetionet}.
The structure of dataset is similar to synthethic dataset - heterogenous with multi-label enpoints, that's 
why it was the choice for running benchmark on it. Even though the structure is similar, it was necessary
to adjust it to node classification task similarly as synthethic dataset. The following adjustement was made
to run benchmark experiment. 

% \begin{enumerate}[label=(\roman*)]
%   \item compound has a role of patient in the graph
%   \item endpoint predicted is disease %to show the % for every endpoint
%   \item mapping done for node categories
%   | Layer | Category | Hetionet value | 
% |---|---|---|---|
% | 1 | `patient_context` | Pharmacologic Class | 
% | 2 | `drug_exposure` | Gene (`CbG`) | 
% | 3 | `mechanism` | Pathway + Biological Process | 
% | 4 | `adr_or_intermediate_state` | Gene (`DaG`) | 
% | 5 | `clinical_endpoint` | Disease | 
%   \item synthethic pipeline is based on direct graphs, so direction was introduced to the graph
%   \item reconstructed the graph structure to represent longer paths (Pharmacological class is joined with gene)
%   \item edge_attr - descrive that it is 
% \end{enumerate}

The benchmark for endpoint prediction is the Hetionet knowledge graph described
in \cref{subsec:hetionet}. Its structure resembles the synthetic dataset ---
heterogeneous, with multi-label targets --- which is why it was selected.
Adapting it to node classification nevertheless required reconstruction listed below. 
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Direction.} Hetionet is largely undirected and
    carries no causal orientation. So every
    edge was oriented from the lower to the higher layer and intra-layer edges
    were discarded, which guarantees acyclicity. The direction is a
    construction decision to check the model. 
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Mapping of Hetionet entities onto the synthethic layer schema.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth, height=0.45\textheight]{rysunki/hetionet_layer_mapping.png}
...
The proxy dataset reproduces the central pattern of the synthetic experiments,
but with one difference that is worth stating carefully. The gain from HCR is
again conditional on the state of the propagation path: at $L=6$ without
...
The difference concerns the interaction between HCR and residual connections.
On the synthetic data the two mechanisms did not add up: once the residual path
restored the flow of information through depth, no HCR variant improved on the
model without it. On the proxy dataset they do combine at $L=6$: the baseline
...
A structural explanation is available. In the synthetic graph the direct
parents of an endpoint are observed, so the HCR branch and the one-hop
receptive field read the same variables and the two mechanisms address the same
bottleneck. In the Hetionet-derived graph the parents that carry per-compound
values sit in layer 2, while layers 3 and 4 are latent relays; information must
therefore traverse several hops, and the residual path and the HCR bypass
contribute at different points of that route. This is consistent with the depth
curve on the proxy dataset, which rises monotonically with $L$ for
\emph{typed+triple} without residual connections ($0.662$, $0.677$, $0.702$,
$0.757$ at $L = 1, 2, 3, 6$) --- the opposite of the flat curve observed on the
synthetic data.

The comparison is limited by unequal coverage: \emph{linear} and
\emph{nonlin.\ all} were only run at $L=3$, and several depths are missing for
individual variants. Differences below roughly $0.03$ AUC fall within the
uncertainty of a single seed at $310$ validation compounds.
...
\begin{table}[H]
  ...
  \caption{Results under full observability on the benchmark dataset.
    \emph{nonlin.\ bin.} uses a 9-channel evidence vector restricted to
    binary parents; \emph{nonlin.\ all} uses 42 channels with the full parent
    scope; \emph{typed+triple} concatenates one encoder per parent type with
    a third-order block. Coverage of depths differs between variants.}
  ...
  \begin{tabular}{ccl SSS SSS}
    ...
    $L$ & Res. & HCR & {AUC} & {AUPRC} & {Lift} & {AUC} & {AUPRC} & {Lift} \\
    \midrule
