...
As edges bring different information based on edge type, they were assigned
declared attributes in the audited graph specification:
...
\begin{itemize}
    ...
    \item \texttt{activation\_frequency} encodes how often the source
    variable is active or clinically relevant in the population.
    \item \texttt{mechanistic\_confidence} encodes the confidence assigned
    during graph audit that the mechanistic link is plausible.
    \item \texttt{evidence\_weight} encodes the weight of supporting
    evidence for the edge in the audited knowledge graph.
\end{itemize}
...
% on the 
%  Second, the three
% endpoints with the lowest oracle AUC (Section~\ref{sec:generation}) --
...
been realised.
Each node value is generated by one of mechanisms:
a stochastic process, or a deterministic computation of other nodes. The following paragraphs
...
A small subset of deterministic variables represents recorded clinical
evidence. Such variables are
computed as the product of an underlying state and its associated observation
process. For example,
...
Thus, a creatinine rise may be present but
...
The observation variables distinguish clinical truth from its documentated representation. They
provide a structured representation of testing and recording processes that
are relevant to the construction and interpretation of alternative
...
For example, the \texttt{selection\_bias} scenario restricts the cohort to
...
All of scenarios are presented in Table ~\ref{tab:synthetic_scenarios}.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}l X@{}}
        ...
        \texttt{no\_overlap} &
        Changed treatment-assignment probabilities: selected drug exposures become rare
        within specific clinical subgroups. \\
        \addlinespace
...
endpoint names, their direct parents in the final DAG, and the
generator-based oracle AUC on the complete synthetic cohort (each patient
scored with the known generative logistic model; relative performance is
reported later via \cref{eq:pct-ceiling}).
...
Patients were partitioned into training, validation, and
