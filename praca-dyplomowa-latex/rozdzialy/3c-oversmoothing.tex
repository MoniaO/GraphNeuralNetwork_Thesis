...
Residual connections (use_residual) switch the effective per-layer propagation operator from the (nilpotent, on a DAG without self-loops) 
...
PairNorm (Zhao & Akoglu, 2020) re-centers and re-scales node representations after each layer to keep the pairwise distance between representations from collapsing, without preventing the underlying mixing.
