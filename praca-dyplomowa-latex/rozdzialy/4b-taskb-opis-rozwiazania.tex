...
shared across relation types but parameterized identically for every patient graph. The following
convolution backbones 
are supported through a common registry and were benchmarked against each other: 
GraphSAGE, R-GCN, and TransformerConv. 
...
Table~\ref{tab:fusion_modes} and below they is included detailed explanation
...
For a binary parent \(R\) and binary endpoint node \(Y\), the primary association
...
% representation (Table~\ref{tab:fusion_modes}).
...
%used to relate architectural choices. 
 ...
 at $0.5$. It is \emph{not} a Bayes-optimal error bound; it is a
 simulation-based performance ceiling derived from the generative model.
